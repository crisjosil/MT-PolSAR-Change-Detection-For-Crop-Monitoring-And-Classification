...
for img in range(N): ####### open image of date i and fix it   
    ...
    for img_2 in range(N): ####### open image of date j, looping from zero to the nth image     
        ...
        Tc = (T22 - T11)      
        List_RGB = eigen.gral_eigendecomposition(Tc)
...
layer_name = "CM_Datacube_viz_alb" #
...
location="D:\Paper2\Ascending_only_V1\\np_arrays\\Change_matrix\\V1\\"
layer_name = "CM_Datacube_with_diag_viz_alb" #
np.save(location+layer_name, CM_Datacube, allow_pickle=True, fix_imports=True)   
...
#Tc = T22 -T11
#application = "Difference change detection"
#eigen = eigendecompositions(application)
#List_RGB = eigen.gral_eigendecomposition(Tc)
#
#R_inc, G_inc, B_inc = List_RGB[0],List_RGB[1],List_RGB[2]
#R_dec, G_dec, B_dec = List_RGB[3],List_RGB[4],List_RGB[5]
#
#
#a=np.zeros((R_dec.shape[0],R_dec.shape[1],3))
#b=np.zeros((R_dec.shape[0],R_dec.shape[1],3))
#fig,(ax,ax1)=plt.subplots(nrows=1,ncols=2,sharex=True,sharey=True,figsize=(15,9))
#a[:,:,0] = R_inc
#a[:,:,1] = G_inc
#a[:,:,2] = B_inc
...
#application = "Single image"
#eigen = eigendecompositions(application)
#R, G, B = eigen.gral_eigendecomposition(T22)
#
#a=np.zeros((R.shape[0],R.shape[1],3))
